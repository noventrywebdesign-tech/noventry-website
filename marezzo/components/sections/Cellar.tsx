...
import { cellarHighlights, pairings } from "@/lib/cellar-data";
  ...
  return (
    <section id="keller" className="relative scroll-mt-20 overflow-hidden bg-midnight-950 px-6 py-24 md:px-16 md:py-36">
      ...
      <div className="relative mx-auto grid max-w-[1600px] grid-cols-1 items-center gap-14 md:grid-cols-2 md:gap-10">
        <div>
          <Reveal>
            <p className="eyebrow mb-6">07 / Nach dem Feuer</p>
          </Reveal>
      ...
      <div className="relative mx-auto mt-20 grid max-w-[1600px] grid-cols-1 gap-14 border-t border-linen-50/10 pt-16 md:grid-cols-2 md:gap-10">
        ...
        <div>
          <Reveal>
            <p className="mb-6 text-[0.65rem] font-medium uppercase tracking-[0.2em] text-linen-400">Empfehlungen der Sommelière</p>
          </Reveal>
          <div className="space-y-7">
            {pairings.map((p, i) => (
              <Reveal key={p.occasion} delay={i * 0.05}>
                <p className="font-display text-xl text-linen-50">{p.occasion}</p>
                <p className="mt-1.5 max-w-md text-sm leading-relaxed text-linen-300">{p.recommendation}</p>
              </Reveal>
            ))}
          </div>
        </div>
