...
// layerRange pads the range to the full [0,1] domain rather than just
// [segStart, segEnd] — see its doc comment for the stale-opacity bug that
// relying on out-of-range clamping caused.
function CutImage({ index, progress }: { index: number; progress: MotionValue<number> }) {
  ...
  const scale = useTransform(progress, range, output.map((o) => (o === 1 ? 1 : 1.04)));
...
// Isolated on purpose, same reasoning as components/sections/Journey.tsx's
// JourneyStepText: this is the only component that calls setState off scroll
// position, so it's the only one that re-renders as the user scrolls. Text is
// driven by a *discrete* active-index state rather than a continuous
// scroll-linked opacity curve — two continuous per-index text curves both
// centered on the same boundary point are numerically supposed to be
// non-overlapping, but in practice a full block of copy (unlike a photo)
// reads as garbled double-exposed text for any measurable overlap window at
// all. Collapsing to "exactly one active index, swapped via a fast,
// self-contained AnimatePresence transition" removes the possibility
// entirely instead of tuning zone widths. Keeping that state in its own leaf
// component also means the sibling CutImage/CutDot layers never re-render
// (and never re-invoke useTransform) as the user scrolls.
function SignatureCutsText({ progress }: { progress: MotionValue<number> }) {
  ...
  const cut = signatureCuts[activeIndex];
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          ...
          <div className="-mt-16 md:-mt-24">
            ...
            <p className="mt-4 text-xs uppercase tracking-[0.14em] text-linen-400">{cut.doneness}</p>
            <p className="mt-3 font-display text-3xl text-linen-50">{cut.price}</p>
          </div>
        </motion.div>
      </AnimatePresence>
    </div>
    ...
    <section ref={ref} className="relative h-[400vh] bg-char-950">
      <div className="sticky top-0 h-screen overflow-hidden">
        <div className="absolute left-6 top-8 z-10 md:left-16 md:top-10">
          <p className="eyebrow">03 / Signature Cuts</p>
        </div>

        <div className="grid h-full grid-cols-1 md:grid-cols-2">
          <SignatureCutsText progress={scrollYProgress} />
